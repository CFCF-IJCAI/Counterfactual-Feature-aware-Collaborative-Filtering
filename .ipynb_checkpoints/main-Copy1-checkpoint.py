...
def parameter_parser():
    ...
    parser.add_argument("--intervener_feature_number", type=int,   default=20,                              help="recommending how many items to a user")
    parser.add_argument("--intervener_iteration",      type=int,   default=200,                             help="number of training epochs")
    parser.add_argument("--intervener_learning_rate",  type=float, default=0.1,                             help="learning rate")
    ...
    parser.add_argument("--intervener_l1_reg",            type=float, default=0.01,                            help="the regular item of the MF model")
...
class experiment():
    ...
    def counter_data_generation_batch(self, user, user_feature, pos_item, pos_item_feature, neg_item, neg_item_feature):
        ...
        for i in range(self.args.intervener_iteration):
            ...
            loss, _ = self.intervener(user, user_feature, pos_item, pos_item_feature, neg_item, neg_item_feature)
            loss.backward(retain_graph=True)
            assert (len(self.intervener.named_parameters()) == 1)
            
            self.intervener_optimizer.step()

        loss, conf = self.intervener(user, user_feature, pos_item, pos_item_feature, neg_item, neg_item_feature)
    ...
    def run(self, mode):
        if mode == 1:
            ...
            gen_train_pos_items_feature, gen_train_neg_items, gen_train_neg_items_feature = self.counter_data_generation(self.anchor_model)
