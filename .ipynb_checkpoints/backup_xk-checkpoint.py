    ...
    def start(self, grid_param_dict):
        ...
        for key, vals in grid_param_dict.items():
            ...
            with mp.Pool(self.n_pool) as p:
                ...
                for ret_key, ret_val in zip(output_keys, output_vals):
                    print ("[Summary]{}:{}:\n\t{}".format(key, val, ret_val))
...
class IntervenerBalancedTaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Digital_Music/ --anchor_model 4 --intervener_batch_size 600 "
...
class Intervener4TaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 4 --intervener_batch_size 11900 --intervener_soft True "
    def default_start(self):
...
class IntervenerTaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 2 --intervener_batch_size 12000 "
    def default_start(self):
        self.best_param_dict = {'confidence': 0.693, 'intervener_learning_rate': 0.0001, 'intervener_reg': 0.0001, 'reg': 0.0025, 'learning_rate': 0.001}
        default_param_dict = {
...
class SoftOnlyTaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 2 --intervener_batch_size 12000 --intervener_soft True "
    def default_start(self):
        self.best_param_dict =  {'confidence': 0.693, 'intervener_learning_rate': 0.5, 'intervener_reg': 0.0001, 'reg': 0.0001, 'learning_rate': 0.01, 'intervener_l1_reg': 0.0025}
        default_param_dict = {
            'confidence': [0.693],
...
class WithoutPerturbationTaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 4 --intervener_batch_size 11000 --intervener_soft True --intervener_l1_reg 0.0" 
    def default_start(self):
        self.best_param_dict =  {'confidence': 0.693, 'intervener_learning_rate': 0.5, 'intervener_reg': 0.0001, 'reg': 0.0001, 'learning_rate': 0.01, 'intervener_l1_reg': 0.0025}
        default_param_dict = {
...
class WithoutNoiseControlTaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 2 --intervener_batch_size 12000 --confidence 1000.0" 
    def default_start(self):
        self.best_param_dict =  {'confidence': 0.693, 'intervener_learning_rate': 0.5, 'intervener_reg': 0.0001, 'reg': 0.0001, 'learning_rate': 0.01, 'intervener_l1_reg': 0.0025}
        default_param_dict = {
            'intervener_learning_rate': [0.5],
...
class ParameterAnalysis10TaskPool(AnchorTaskPool):
    ...
    def default_start(self):
        self.best_param_dict = {'confidence': 0.65, 'intervener_learning_rate': 0.001, 'intervener_reg': 1.5, 'reg': 0.0125, 'learning_rate': 0.01, 'intervener_l1_reg': 0.0001, 'intervener_feature_number': 100}
...
class ParameterAnalysis12TaskPool(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Office_Products/ --anchor_model 4 --intervener_batch_size 11000 --intervener_soft True "

    def default_start(self):
        #assert False, "Input the best_param_dict"
        self.best_param_dict = {'confidence': 0.65, 'intervener_learning_rate': 0.3, 'intervener_reg': 0.0001, 'reg': 0.0025, 'learning_rate': 0.01, 'intervener_l1_reg': 0.0025}

        default_param_dict = {
            'intervener_l1_reg': [0.0001, 0.0005, 0.0025, 0.0125, 0.0625, 0.3, 1.5, 4.5, 13.5, 60],
        }
        self.start(default_param_dict)
...
class CaseFindTask(AnchorTaskPool):
    def _cmd_body(self):
        return "python main.py --data_path ./data/Amazon_Instant_Video/ --anchor_model 2 --intervener_batch_size 700 "
    ...
    def default_start(self):
        ...
        default_param_dict = {
            "case_model": range(0, 130), 
        }
        self.start(default_param_dict)
    
    
id2task = {
    0: CaseFindTask(5, "long"), # case find. need range from [0, #Feature]
    
    6: IntervenerTaskPool(10, 'long'), # hard
    3: Intervener4TaskPool(10, 'long'),  # soft + balanced
    2: Intervener3TaskPool(10, 'long'),  # random noise
    ...
    10:ParameterAnalysis10TaskPool(4, 'long'), # confident changes
    11:ParameterAnalysis11TaskPool(4, 'long'), # K changes
    12:ParameterAnalysis12TaskPool(4, 'long'), # beta changes
}
